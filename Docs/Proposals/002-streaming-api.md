...
protocol LLM {
  ...
  ) async throws -> AsyncStream<T.Partial>
...
The `@Generable` macro automatically generates a nested `Partial` type for streaming support:
...
let stream = try await llm.replyStream(
