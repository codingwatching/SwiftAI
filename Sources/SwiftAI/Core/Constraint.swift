...
extension Constraint {
    ...
    public static func count<Element>(_ count: Int) -> Constraint<[Element]> where Value == [Element] {
        Constraint(kind: .array(.count(lowerBound: count, upperBound: count)))
    }
    
    /// Sets a minimum number of elements in an array.
    public static func minimumCount<Element>(_ count: Int) -> Constraint<[Element]> where Value == [Element] {
        Constraint(kind: .array(.count(lowerBound: count, upperBound: nil)))
    }
    
    /// Sets a maximum number of elements in an array.
    public static func maximumCount<Element>(_ count: Int) -> Constraint<[Element]> where Value == [Element] {
        Constraint(kind: .array(.count(lowerBound: nil, upperBound: count)))
    }
    
    /// Applies a constraint to each element in the array.
    public static func element<Element>(_ constraint: Constraint<Element>) -> Constraint<[Element]> where Value == [Element] {
